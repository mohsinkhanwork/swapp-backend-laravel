        ...
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-8">
                    <div class="row mb-4">
                        <div class="col-8">
                            <h4>{{__('dashboard.skills')}}</h4>
                        </div>
                        <div class="col-4 text-end">
                            <a class="btn btn-success" href="{{route('admin.skills.create')}}">{{__('dashboard.add-new')}}</a>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach ($skills as $skill)
                                <tr>
                                    ...
                                    <td>{{$skill->title_tr}}</td>
                                    <td>{{$skill->category->title_en??'--'}}</td>
                                    <td>
                                        <a class="btn btn-info btn-icon" href="{{route('admin.skills.edit',$skill->id)}}">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{route('admin.skills.destroy',$skill->id)}}" class="d-inline" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger btn-icon m-2 delete-skill" type="button">
                                                <i class="fa-solid fa-trash"></i>
                                            </button>
                                        </form>
                                        @can('skill-quiz')
                                            <a class="btn btn-secondary" href="{{route('admin.skill-quiz',$skill->id)}}">
                                                {{__('dashboard.manage-quiz')}}
                                            </a>
                                        @endcan
                                    </td>
                                </tr>
