                ...
                <div class="widget-content widget-content-area br-8">
                    <div class="row mb-4">
                        ...
                        <div class="col-4 text-end">
                            <a class="btn btn-success" href="{{route('admin.blogs.create')}}">{{__('dashboard.add-new')}}</a>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach ($blogs as $blog)
                                <tr>
                                    <td>{{$blog->id}}</td>
                                    <td>{{$blog->title}}</td>
                                    <td>{{$blog->category->name??'Others'}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <a class="btn btn-info btn-icon" href="{{route('admin.blogs.edit',$blog->id)}}">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{route('admin.blogs.destroy',$blog->id)}}" class="d-inline" method="post">
                                            @csrf
                                            @method('DELETE')
