            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="row">
                    ...
                    <div class="col-12 text-end mb-2">
                        <a href="{{route('admin.support')}}" class="btn btn-info">{{__('dashboard.go-back')}}</a>
                    </div>
                    <div class="col-12">
                        <div class="widget-content widget-content-area br-8 mb-3">
                            <h4 class="mb-4">{{__('dashboard.subject')}} : {{$ticket->subject}}</h4>
                            <p class="mb-1 fw-bold">{{__('dashboard.user-details')}}: </p>
                            <div class="mb-3 ms-3">
                                <p class="mb-0">
                            ...
                            <p>
                                <strong class="fw-bold">{{__('dashboard.date')}}: </strong>
                                {{$ticket->created_at->format('d M, Y g:i A')}}
                            </p>
                            <p>
                                <strong class="fw-bold">{{__('dashboard.status')}}: </strong>
                                {{$ticket->status}}
                            </p>
                            <p>
                                <strong class="fw-bold">{{__('dashboard.priority')}}: </strong>
                                {{$ticket->priority}}
                            </p>
                            <p> <strong class="fw-bold"> {{__('dashboard.description')}}: </strong></p>
                            <div>
                                {!!$ticket->description!!}
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="widget-content widget-content-area br-8">
                            <form action="{{route('admin.support.reply',$ticket->ticket_number)}}" method="post" enctype="multipart/form-data">
                                @csrf
